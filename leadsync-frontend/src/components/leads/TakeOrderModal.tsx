      ...
      {isOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          ...
          <motion.div
            ...
            className="bg-white rounded-[24px] shadow-2xl w-full max-w-lg overflow-hidden relative z-10 border border-slate-100 flex flex-col font-sans select-none"
          >
            ...
            <div className="bg-gradient-to-r from-[#0047cc] to-blue-600 p-6 text-white text-left relative">
              <button
                type="button"
                onClick={onClose}
                className="absolute top-5 right-5 p-1 text-white/80 hover:text-white hover:bg-white/10 rounded-lg transition"
              >
                <X size={18} />
              </button>
            ...
            <form onSubmit={handleCreateOrderSubmit} className="p-6 space-y-4 max-h-[80vh] overflow-y-auto text-left">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                  ...
                  <input
                    ...
                    value={agentName}
                    className="w-full px-3.5 py-2.5 bg-slate-100 border border-slate-200 rounded-xl text-slate-500 text-xs font-semibold focus:outline-none cursor-not-allowed select-none font-sans"
                  />
                </div>
                  ...
                  {selectedMenuItems.length > 0 ? (
                    <div className="space-y-1.5 pt-2 max-h-[160px] overflow-y-auto">
                      {selectedMenuItems.map((item, idx) => (
                        <div key={idx} className="flex items-center justify-between bg-white border border-slate-100 px-3 py-1.5 rounded-lg text-xs font-semibold text-slate-700 shadow-sm animate-in fade-in slide-in-from-top-1 duration-100">
              ...
              <div className="flex gap-3 justify-end pt-5 border-t border-slate-100">
                <button
                  type="button"
                  onClick={onClose}
                  className="px-4 py-2.5 text-slate-500 hover:bg-slate-50 rounded-xl text-xs font-bold border border-slate-200 active:scale-95 transition font-sans cursor-pointer"
                >
                  Cancel
                </button>
                <button
