    ...
    <div>
      <div className="flex items-center justify-between mb-4">
        <div className="flex items-center gap-2">
          ...
          <h2 className="text-lg font-bold text-slate-900">Automation Rules</h2>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-2">
            {TEMPLATES.map(t => (
              <button
                ...
                className="text-left bg-slate-50 hover:bg-indigo-50 border border-slate-200 hover:border-indigo-200 rounded-xl p-3 transition group"
              >
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  ...
                  <select
                    ...
                    className="w-full border border-slate-200 rounded-lg px-3 py-2 text-sm bg-white focus:outline-none focus:ring-2 focus:ring-amber-300"
                  >
              ...
              {newAction === "SEND_MESSAGE" && (
                <div>
                  <textarea
                    ...
                    rows={3}
                    maxLength={500}
                    className="w-full border border-slate-200 rounded-lg px-3 py-2 text-sm bg-white focus:outline-none focus:ring-2 focus:ring-amber-300 resize-none"
                  />
          ...
          <AnimatePresence>
            {rules.map(rule => (
              <motion.div
                ...
                exit={{ opacity: 0, x: -20 }}
                className={`border rounded-xl p-3.5 transition-all ${rule.isActive ? "bg-white border-slate-200" : "bg-slate-50 border-slate-100 opacity-60"}`}
              >
                <div className="flex items-start justify-between gap-3">
                  <div className="flex-1 min-w-0">
                    <p className="text-sm font-semibold text-slate-800 mb-1.5">{rule.name}</p>
                    <div className="flex flex-wrap gap-1.5">
                      <span className={`text-[10px] font-bold px-2 py-0.5 rounded border ${TRIGGER_COLOR[rule.trigger] ?? "bg-slate-100 text-slate-600 border-slate-200"}`}>
                        {TRIGGER_LABELS[rule.trigger] ?? rule.trigger}
                      </span>
                      <span className="text-[10px] text-slate-400 flex items-center gap-0.5 bg-slate-50 border border-slate-200 px-2 py-0.5 rounded">
                        <Clock size={9} />
                        {rule.triggerDelayMinutes >= 60 ? `${Math.round(rule.triggerDelayMinutes / 60)}h` : `${rule.triggerDelayMinutes}m`}
                      </span>
                      <span className={`text-[10px] font-bold px-2 py-0.5 rounded border ${ACTION_COLOR[rule.action] ?? "bg-slate-100 text-slate-600 border-slate-200"}`}>
