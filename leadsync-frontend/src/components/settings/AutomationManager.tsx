...
const TRIGGER_COLOR: Record<string, string> = {
  LEAD_COLD: "bg-blue-500/10 text-blue-400 border-blue-500/20",
  ORDER_PENDING: "bg-amber-500/10 text-amber-400 border-amber-500/20",
  NEW_LEAD: "bg-emerald-500/10 text-emerald-400 border-emerald-500/20",
};

const ACTION_COLOR: Record<string, string> = {
  SEND_MESSAGE: "bg-indigo-500/10 text-indigo-400 border-indigo-500/20",
  CHANGE_SEGMENT: "bg-violet-500/10 text-violet-400 border-violet-500/20",
};
    ...
    <div>
      <div className="flex items-center justify-between mb-4">
        <div className="flex items-center gap-2">
          ...
          <h2 className="text-lg font-bold text-text-primary">Automation Rules</h2>
      ...
      {canEdit && rules.length === 0 && !showForm && (
        <div className="mb-5">
          <p className="text-xs font-bold text-text-muted uppercase tracking-wider mb-2">Quick Templates</p>
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-2">
            {TEMPLATES.map(t => (
              <button
                key={t.name}
                onClick={() => applyTemplate(t)}
                className="text-left bg-background-tertiary hover:bg-indigo-500/10 border border-border hover:border-indigo-500/20 rounded-xl p-3 transition group"
              >
                <div className="flex items-center gap-1.5 mb-1">
                  <Play size={11} className="text-indigo-400 group-hover:text-indigo-600" />
                  <span className="text-xs font-bold text-text-secondary group-hover:text-indigo-400">{t.name}</span>
                </div>
                <p className="text-[10px] text-text-disabled leading-relaxed line-clamp-2">
      ...
      <AnimatePresence>
        {showForm && (
          ...
          >
            <div className="bg-amber-500/10 border border-amber-500/20 rounded-xl p-4 mb-4 space-y-3">
              <input
                type="text"
                placeholder="Rule name — e.g. 'Re-engage cold leads'"
                value={newName}
                onChange={e => setNewName(e.target.value)}
                className="w-full border border-border bg-background-tertiary rounded-lg px-3 py-2 text-sm text-text-primary placeholder:text-text-disabled focus:outline-none focus:ring-2 focus:ring-amber-400"
              />
              <div className="grid grid-cols-2 gap-3">
                <div>
                  <label className="text-[10px] font-bold text-text-muted uppercase tracking-wider block mb-1">Trigger</label>
                  <select
                    value={newTrigger}
                    onChange={e => setNewTrigger(e.target.value)}
                    className="w-full border border-border bg-background-tertiary rounded-lg px-3 py-2 text-sm text-text-primary focus:outline-none focus:ring-2 focus:ring-amber-400"
                  >
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  <label className="text-[10px] font-bold text-text-muted uppercase tracking-wider block mb-1">Action</label>
                  <select
                    value={newAction}
                ...
                {newAction === "CHANGE_SEGMENT" && (
                  <div>
                    <label className="text-[10px] font-bold text-text-muted uppercase tracking-wider block mb-1">Segment</label>
                    <select
                      value={newSegment}
              ...
              {newAction === "SEND_MESSAGE" && (
                <div>
                  <textarea
                    ...
                    rows={3}
                    maxLength={500}
                    className="w-full border border-border bg-background-tertiary rounded-lg px-3 py-2 text-sm text-text-primary placeholder:text-text-disabled focus:outline-none focus:ring-2 focus:ring-amber-400 resize-none"
                  />
                  <div className="text-right text-[10px] text-text-disabled -mt-1">{newMessage.length}/500</div>
                </div>
              )}
      ...
      {loading ? (
        <div className="space-y-3">
          {[1, 2].map(i => <div key={i} className="h-16 bg-background-tertiary rounded-xl animate-pulse" />)}
        </div>
          ...
          <AnimatePresence>
            {rules.map(rule => (
              <motion.div
                ...
                exit={{ opacity: 0, x: -20 }}
                className={`border rounded-xl p-3.5 transition-all ${rule.isActive ? "bg-background-secondary border-border" : "bg-background-tertiary border-border opacity-60"}`}
              >
                <div className="flex items-start justify-between gap-3">
                  <div className="flex-1 min-w-0">
                    <p className="text-sm font-semibold text-text-primary mb-1.5">{rule.name}</p>
                    <div className="flex flex-wrap gap-1.5">
                      <span className={`text-[10px] font-bold px-2 py-0.5 rounded border ${TRIGGER_COLOR[rule.trigger] ?? "bg-background-elevated text-text-muted border-border"}`}>
                        {TRIGGER_LABELS[rule.trigger] ?? rule.trigger}
                      </span>
                      <span className="text-[10px] text-text-disabled flex items-center gap-0.5 bg-background-tertiary border border-border px-2 py-0.5 rounded">
                        <Clock size={9} />
                        {rule.triggerDelayMinutes >= 60 ? `${Math.round(rule.triggerDelayMinutes / 60)}h` : `${rule.triggerDelayMinutes}m`}
                      </span>
                      <span className={`text-[10px] font-bold px-2 py-0.5 rounded border ${ACTION_COLOR[rule.action] ?? "bg-background-elevated text-text-muted border-border"}`}>
                        {ACTION_LABELS[rule.action] ?? rule.action}
                      </span>
                      {rule.runCount > 0 && (
                        <span className="text-[10px] text-text-disabled bg-background-tertiary border border-border px-2 py-0.5 rounded">
                          ran {rule.runCount}×
                        </span>
                      )}
                    </div>
                  ...
                  {canEdit && (
                    <div className="flex items-center gap-1.5 shrink-0">
                      <button
                        ...
                        className="text-text-disabled hover:text-amber-400 transition"
