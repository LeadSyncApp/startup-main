...
export const CustomerList: React.FC = () => {
  const { user } = useAuth();
  const canDeleteCustomer = user?.role === 'MANAGER' || user?.role === 'OWNER';
  ...
  const [deletingCustomer, setDeletingCustomer] = useState<Lead | null>(null);
  ...
  const handleConfirmDeleteCustomer = async () => {
    ...
    setIsDeleting(true);
    try {
      const res = await authedFetch(`/api/leads/${deletingCustomer.id}`, {
        method: 'DELETE',
      });
      if (!res.ok) {
      ...
      toast.success(`Customer ${deletingCustomer.name || deletingCustomer.contact} deleted`);
                  ...
                  >
                    ...
                    <td className="p-4">
                      <div className="flex items-center gap-1">
                        {canDeleteCustomer && (
                          <button
                            onClick={() => setDeletingCustomer(lead)}
                            className="p-1.5 text-slate-300 hover:text-rose-600 transition rounded-lg hover:bg-rose-50 cursor-pointer"
                            title="Delete Customer"
                          >
                            <Trash2 className="size-4" />
                          </button>
                        )}
                        <button className="text-slate-300 hover:text-slate-600 transition p-1.5">
            ...
            >
              <div className="flex items-center gap-3 mb-4">
                ...
                <div>
                  ...
                  <p className="text-xs text-slate-500 font-medium">{deletingCustomer.name || deletingCustomer.contact}</p>
                </div>
              </div>

              <p className="text-xs text-slate-600 font-medium leading-relaxed bg-rose-50/70 border border-rose-100 p-3 rounded-2xl mb-6">
                This will permanently delete this customer and all their history. This cannot be undone.
              </p>
              ...
              <div className="flex items-center justify-end gap-3">
                ...
                <button
                  ...
                  disabled={isDeleting}
                  className="px-4 py-2 bg-rose-600 hover:bg-rose-700 disabled:opacity-50 text-white rounded-xl text-xs font-black transition cursor-pointer shadow-md shadow-rose-600/20"
                >
