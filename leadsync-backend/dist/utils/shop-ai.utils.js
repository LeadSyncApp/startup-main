...
exports.getMenuSnapshot = getMenuSnapshot;
exports.calculateRetrieval = calculateRetrieval;
exports.safeJsonParse = safeJsonParse;
exports.sanitizeReply = sanitizeReply;
// In-memory session store (Phase 1)
const sessions = new Map();
function getSession(tenant_id, chat_id) {
...
function updateSession(tenant_id, chat_id, updates) {
    const key = `${tenant_id}:telegram:${chat_id}`;
    const current = getSession(tenant_id, chat_id);
    sessions.set(key, { ...current, ...updates });
}
function getMenuSnapshot(structuredMenu) {
    if (!structuredMenu || !structuredMenu.categories) {
        return { categories: [] };
    }
    return {
        categories: structuredMenu.categories.map((cat) => ({
            name: cat.name,
...
function calculateRetrieval(query, menuSnapshot) {
    const tokens = query.toLowerCase().split(/\s+/).filter(t => t.length > 2);
    const items = [];
    menuSnapshot.categories.forEach((cat) => {
