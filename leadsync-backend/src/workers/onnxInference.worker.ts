...
// Gate onnxruntime-node loading behind env var check to prevent native binary crash
if (process.env.ONNX_DISABLE === "true" || process.env.SKIP_ONNX_WARMUP === "true") {
  console.log("[OnnxWorker] ONNX disabled via env var — worker exiting.");
  process.exit(0);
}

(async () => {
  // MANDATORY: Polyfill self before importing @xenova/transformers
