...
async function testUseAIFallthrough() {
  const companyId = "6e91a188-f794-4c59-b367-44b9db07b10f";
  let ruleId: string | null = null;
  let conversationId: string | null = null;

  // Intercept fetch to inspect outbound messages sent by the AI
  ...
  try {
    ...
    await tenantContextStorage.run(contextStore, async () => {
      // 2. Simulates a free-typed customer message matching the keyword "saffron"
      const context = {
        companyId,
        conversationId: conversation.id,
        leadId: lead.id,
      ...
      // 3. Confirm evaluateMessage returns matched=false (so it falls through)
      ...
      // Clean the rules cache to make sure the orchestrator sees the new rule
      ...
      // 5. Confirm response is NOT raw templateBody verbatim, but shows dynamic elaboration
      const rawCanned = "Canned Saffron response: We sell Saffron.";
      if (interceptedMessage.trim() === rawCanned) {
        throw new Error("❌ FAILURE: Sent the canned templateBody verbatim!");
      }

      const containsSaffron = interceptedMessage.toLowerCase().includes("saffron") || interceptedMessage.toLowerCase().includes("kesar");
      const containsPrice = interceptedMessage.includes("300");
    ...
    if (ruleId) {
      await prisma.conversationalRule.delete({ where: { id: ruleId } }).catch(console.error);
    }
    if (conversationId) {
      await prisma.conversation.delete({ where: { id: conversationId } }).catch(console.error);
    }
    console.log("Cleanup complete. Disconnecting Prisma...");
    await prisma.$disconnect();
  }
}

testUseAIFallthrough();
