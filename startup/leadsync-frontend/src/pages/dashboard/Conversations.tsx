...
export default function Conversations() {
  ...
  return (
    <PageTransition className="flex bg-white rounded-3xl overflow-hidden shadow-2xl h-[calc(100vh-160px)] min-h-[500px]">
          ...
          <div className="flex items-center justify-between">
            ...
            <div className="flex items-center gap-1">
              {conversations.length > 0 && (
                <span className="text-[10px] font-black bg-indigo-50 text-indigo-600 px-2 py-0.5 rounded-full mr-2">
              ...
              {filteredConversations.map((conv) => (
                <motion.div
                  ...
                  className={`
                    ...
                    ${selected?.id === conv.id ? "bg-white shadow-lg shadow-indigo-500/5 z-10 border-l-2 border-indigo-500" : "hover:bg-indigo-50/50 border-l-2 border-transparent"}
                  `}
                >
          ...
          <>
            ...
            <div className="px-8 py-4 bg-white border-b border-slate-100 flex items-center justify-between z-10 shadow-sm">
              <div className="flex items-center gap-4">
                <button onClick={() => setShowMobileList(true)} className="lg:hidden p-2.5 bg-slate-100 rounded-xl text-slate-600 hover:bg-slate-200 active:scale-95 transition">
                  <ArrowLeft size={18} />
                </button>
                ...
                <div className="hidden md:flex bg-slate-100 p-1 rounded-xl border border-slate-200">
                  <button
                    ...
                    className={`flex items-center gap-1.5 px-3 py-1 rounded-lg text-[10px] font-black transition-all uppercase tracking-widest ${
                      chatTab === "messages" ? "bg-white text-indigo-600 shadow" : "text-slate-500"
                    }`}
                  >
                  ...
                  <button
                    ...
                    className={`flex items-center gap-1.5 px-3 py-1 rounded-lg text-[10px] font-black transition-all uppercase tracking-widest ${
                      chatTab === "notes" ? "bg-white text-amber-600 shadow" : "text-slate-500"
                    }`}
                  >
            ...
            <div className="flex-1 flex overflow-hidden">
              <div className="flex-1 flex flex-col min-w-0 h-full relative">
                ...
                {/* CHAT INPUT AREA */}
                {isLocked ? (
                  <div className="p-4 lg:p-8 bg-slate-50 border-t border-slate-200 text-center">
                    ...
                    <div className="relative">
                      ...
                      <AnimatePresence>
                        {typingAgent && (
                          ...
                          >
                            <div className="flex items-center gap-1.5 bg-slate-100 text-slate-500 text-[11px] font-medium px-3 py-1.5 rounded-full border border-slate-200">
                      ...
                      <div className="flex items-center gap-2 lg:gap-3 bg-slate-100/50 p-2 lg:p-2.5 rounded-[2rem] border border-slate-200/60 shadow-inner focus-within:border-indigo-500/50 focus-within:bg-white transition-all">
                        {/* Saved replies trigger button */}
                        <button
                          type="button"
                          onClick={() => { setShowReplies(s => !s); setRepliesQuery(""); }}
                          title="Saved replies (or type /)"
                          className="shrink-0 h-8 w-8 flex items-center justify-center rounded-full text-slate-400 hover:text-indigo-600 hover:bg-indigo-50 transition ml-1"
                        >
                          <BookOpen size={15} />
                        </button>
                        {/* AI Suggest Reply */}
                        ...
                        <textarea
                          ...
                          className="flex-1 bg-transparent border-none resize-none py-2.5 lg:py-3 px-2 lg:px-4 text-sm focus:ring-0 max-h-32 custom-scrollbar font-medium text-slate-700 placeholder:text-slate-400"
                        />
                        <button onClick={() => { sendMessage(); setShowReplies(false); }} disabled={!newMessage.trim()} className={`h-10 w-10 flex items-center justify-center rounded-full transition-all shadow-xl ${newMessage.trim() ? "bg-indigo-600 text-white scale-100 hover:bg-indigo-700 hover:rotate-12" : "bg-slate-200 text-slate-400 scale-90 cursor-not-allowed"}`}>
                          <Send size={18} />
                        </button>
                      </div>
                    </div>
